...
-- Add new control-plane fields to drift_plans table (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'drift_plans') THEN
...
-- Update existing plans with default values (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'drift_plans') THEN
    UPDATE "drift_plans"
